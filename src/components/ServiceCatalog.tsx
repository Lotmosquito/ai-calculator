...
export function ServiceCatalog({ isOpen, onClose, onSelectService, services }: ServiceCatalogProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-40">
      <div className="service-catalog-panel fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-xl z-50 transform transition-transform duration-300 ease-in-out" style={{ transform: isOpen ? 'translateX(0)' : 'translateX(100%)' }}>
              ...
              <div key={category} className="mb-4">
                ...
                <div className="space-y-2">
                  {servicesInCategory.map(service => (
                    <div key={service.id} onClick={() => onSelectService(service)} className="p-3 border rounded-lg cursor-pointer hover:shadow-md" style={{ borderColor: 'var(--hint-color)' }}>
