...
export function PlanetSelector({ onSelect }: { onSelect: (planet: 'earth' | 'mars') => void; }) {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 text-center">
      ...
      <p className="mb-8 text-sm" style={{ color: 'var(--hint-color)' }}>Это определит базовые цены и сложность работы.</p>
      <div className="w-full max-w-md space-y-4">
        ...
        >
          ...
          <p className="text-sm font-normal" style={{ color: 'var(--hint-color)' }}>Выполнение конкретной услуги от частного мастера. Идеально для тестов, личных проектов и быстрого результата.</p>
        </button>
        <button
        ...
        >
          ...
          <p className="text-sm font-normal" style={{ color: 'var(--hint-color)' }}>Комплексный проект под ключ. Включает стратегию, команду и юридические гарантии. Идеально для бизнеса.</p>
        </button>
      </div>
    </div>
  );
